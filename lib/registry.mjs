...
export function latestPageRecord(user) {
...
export function removePageFromRegistry(registry, userId, pageId) {
  const userIndex = registry.users.findIndex((user) => user.id === userId);
  if (userIndex < 0) {
    return null;
  }

  const user = registry.users[userIndex];
  const pageIndex = user.pages.findIndex((page) => page.id === pageId);
  if (pageIndex < 0) {
    return null;
  }

  const [page] = user.pages.splice(pageIndex, 1);
  const snapshotUser = {
    id: user.id,
    name: user.name,
  ...
  if (user.pages.length === 0) {
    registry.users.splice(userIndex, 1);
    return {
      user: snapshotUser,
      page,
      removedUser: true,
  ...
  return {
    ...
    page,
    removedUser: false,
    nextPage: latestPageRecord(user)
  };
}
