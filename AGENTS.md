...
4. `--user-id` 和 `--user-name` 默认必须取自页面作者所属项目的身份路径：默认是一层 `<project>`，如果页面作者位于受支持的容器目录下（例如 `agentSpace/<child>`），允许提升为两层 `<project>/<child>`；暂时不开放三级目录。禁止使用 `codex`、`agent`、`assistant` 等泛名；`--user` 只能作为两者都取同一身份路径的简写。
...
4. 将页面作者所属项目的身份路径作为 `--user-id` 和 `--user-name`：默认用 `<project>`，若位于受支持的二级容器目录内则用 `<project>/<child>`；不要用 `codex` 之类的泛名，也不要构造三级目录。
...
node scripts/register-page.mjs \
  ...
  --user-id "你的项目名或项目名/子项目名" \
  --user-name "你的项目名或项目名/子项目名" \
